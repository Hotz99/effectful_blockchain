...
import * as BlockService from "./block";
...
export class ValidationError extends Data.TaggedError("ValidationError")<{
  readonly blockIdx: Primitives.PositiveInt;
...
export const BlockchainServiceLive = Layer.effect(
  BlockchainService,
  Effect.gen(function* () {
    const blockService = yield* BlockService.BlockService;
    ...
    return BlockchainService.of({
      ...
      validateChain: (state) => {
        ...
        for (const [prev, curr] of Array.window(blocks, 2)) {
          if (curr.hash !== blockService.computeHash(curr))
            return Either.left(
              new ValidationError({
      ...
      getStats: (state) =>
        makeStats({
          pendingEvents: Option.map(state.pendingEvents, Chunk.size),
          eventsPerBlock: state.eventsPerBlock,
          totalBlocks: Chunk.size(state.chain),
          totalEvents: Chunk.reduce(
            state.chain,
            Option.getOrElse(
              Option.map(state.pendingEvents, Chunk.size),
              () => 0
            ),
