...
Use Context.Tag to register service interface definitions:
...
## No Error Handling in Function Implementations

The idiomatic approach in **Effect** is to let error channel (`E`) express failure and propagate automatically—**no local handling unless semantic translation is required**.

```ts
// `E = ParseResult.ParseError | OtherErrorTypes`
const loadPrivateKeyImpl = (privateKeyHex: string) =>
  Effect.gen(function* () {
    const privateKey = yield* Schema.decode(Key.PrivateKeySchema)(privateKeyHex)
    ...
  })
```

This is correct and idiomatic. From the above signature, downstream callers can explicitly handle or recover via `Effect.catchTag`, `Effect.mapError`, or `Effect.orElse`.

Only intercept locally if:

- we need to **translate** a generic error into a domain error (e.g., wrap `ParseError` in `InvalidPrivateKeyError`), or
- we must perform **cleanup or compensation** at the error site.

Otherwise, allow propagation. The runtime ensures correct short-circuiting and error typing—no try/catch or imperative handling is needed.

### Exhaustive Error Handling (At Call Site)

Use pattern matching to ensure all errors are handled at call site:
...
```typescript
it.effect("should propagate errors properly", () =>
  Effect.gen(function* () {
    const result = yield* Effect.flip(failingOperation())
    assert.instanceOf(result, UserNotFoundError)
  })
)
```
...
function UserForm(props: UserFormProps) {
  ...
  return (
    <>
      ...
      {props.isUpdateUser ? <UpdateHeader /> : <CreateHeader />}
...
export const ThreadComposer: React.FC<{ channelId: string }> = ({
  channelId
}) => {
...
export const EditMessageComposer: React.FC<{ onCancel: () => void }> = ({
  onCancel
}) => {
...
const ForwardButton: React.FC = () => {
  ...
  return <Button onClick={() => Effect.runPromise(submit())}>Forward</Button>
}
```
  ...
  return Result.match(result, {
    ...
    onSuccess: ({ value }) => (
      <>
        ...
        {value.hasMore && (
          <LoadMoreButton loading={value.waiting} onClick={() => loadMore()} />
        )}
      </>
    )
  })
...
function SearchPage() {
  ...
  return (
    <div>
      ...
      />
      {isSearching ? <LoadingSpinner /> : <SearchResults results={results} />}
    </div>
  )
}
...
function ValidatedForm() {
  ...
  return (
    <form>
      ...
      />
      {errors.email && (
        <span style={{ opacity: isPending ? 0.5 : 1 }}>{errors.email}</span>
      )}
      {/* More fields... */}
    </form>
  )
